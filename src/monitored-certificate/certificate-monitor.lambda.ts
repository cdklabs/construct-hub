...
export const handler = metricScope(
  (metrics) => async (event: ScheduledEvent, _context: Context) => {
...
function tlsValidDaysRemaining(endpoint: string, now: Date): Promise<number> {
  console.log(
    `Checking remaining validity time for certificate on ${endpoint}:443`
  );
  ...
  return new Promise<number>((resolve, reject) => {
    ...
    try {
      sock.once('error', (err) =>
        err.code === 'CERT_HAS_EXPIRED' ? ok(0) : ko(err)
      );
      sock.once('secureConnect', () => {
        const cert = sock.getPeerCertificate();
        console.log(
          `Secure connection established: ${cert.fingerprint256} is valid from ${cert.valid_from} until ${cert.valid_to}`
        );
