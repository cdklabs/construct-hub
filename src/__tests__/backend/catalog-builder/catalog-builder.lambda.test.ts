...
test('initial build', () => {
  ...
  const mockFirstPage: AWS.S3.ObjectList = [
    ...
    { Key: `${constants.STORAGE_KEY_PREFIX}@scope/package/v1.2.3${constants.docsKeySuffix('ts')}` },
    ...
    { Key: `${constants.STORAGE_KEY_PREFIX}name/v1.2.3${constants.docsKeySuffix('ts')}` },
  ];
  const mockSecondPage: AWS.S3.ObjectList = [
    ...
    { Key: `${constants.STORAGE_KEY_PREFIX}@scope/package/v1.0.0${constants.docsKeySuffix('ts')}` },
    ...
    { Key: `${constants.STORAGE_KEY_PREFIX}name/v2.0.0-pre${constants.docsKeySuffix('ts')}` },
  ];
  AWSMock.mock('S3', 'listObjectsV2', (req: AWS.S3.ListObjectsV2Request, cb: Response<AWS.S3.ListObjectsV2Output>) => {
    try {
      expect(req.Bucket).toBe(mockBucketName);
