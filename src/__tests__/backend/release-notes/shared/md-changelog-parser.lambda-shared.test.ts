...
describe('changelog parser generates logs', () => {
  ...
  test.each([
    ...
    const expectedChangeLog2 = [...bugfixes1].join('\n');

    await expect(getReleaseNotesMd(changelogMdStr, '1.152.0')).resolves.toEqual(
      expectedChangeLog1
    );

    await expect(getReleaseNotesMd(changelogMdStr, '1.151.0')).resolves.toEqual(
      expectedChangeLog2
    );
  });
  ...
  test.each([
    ...
    'from setext style header for %s',
    async (_heading: string, headingFooter: string) => {
      const features1 = [
        '* **pkgName:** Some feature ([#1234](https://github.com/somerepo/somepkg/issues/1234)) ([dba96a9](https://github.com/somerepo/somepkg/commits/dba96a9)',
        '* **pkgName2:** Some other feature ([#1236](https://github.com/somerepo/somepkg/issues/1236)) ([dba96a1](https://github.com/somerepo/somepkg/commits/dba96a1)',
      ];
      ...
      await expect(
        getReleaseNotesMd(changelogMdStr, '1.152.0')
      ).resolves.toEqual(expectedChangeLog1);

      await expect(
        getReleaseNotesMd(changelogMdStr, '1.151.0')
      ).resolves.toEqual(expectedChangeLog2);
    }
  );
});
