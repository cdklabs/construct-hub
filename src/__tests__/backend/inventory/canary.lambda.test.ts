import { serialize, deserialize, InventoryCanaryState, Grain, DocumentationStatus } from '../../../backend/inventory/canary.lambda';

test('can serialize and deserialize the inventory state', () => {
  // GIVEN
  const state: InventoryCanaryState = {
    continuationToken: 'token',
    indexedPackages: new Map([['package1', {
    ...
    perLanguage: new Map([['python', new Map([[Grain.PACKAGES, new Map([['package1', DocumentationStatus.MISSING]])]])]]),
  };
  ...
  // THEN
  expect(serialized).toEqual(JSON.stringify({
    continuationToken: 'token',
    indexedPackages: {
      _type: 'Map',
      value: [
