...
} from '../../../backend/ingestion/framework-detection.lambda-shared';

describe('AWS CDK frameworks', () => {
  expect(frameworkForDeps(['@aws-cdk/aws-s3@1.134.0'])).toEqual([
    { name: ConstructFrameworkName.AWS_CDK, majorVersion: 1 },
  ]);

  expect(
    frameworkForDeps(['@aws-cdk/aws-s3@1.134.0', '@aws-cdk/aws-lambda@1.134.0'])
  ).toEqual([{ name: ConstructFrameworkName.AWS_CDK, majorVersion: 1 }]);

  expect(
    frameworkForDeps(['@aws-cdk/aws-s3@1.134.0', '@aws-cdk/aws-lambda@1.135.0'])
  ).toEqual([{ name: ConstructFrameworkName.AWS_CDK, majorVersion: 1 }]);

  it('classifies no major version if versions are mixed', () => {
    expect(
      frameworkForDeps(['@aws-cdk/aws-s3@1.134.0', '@aws-cdk/aws-lambda@2.0.0'])
    ).toEqual([
      { name: ConstructFrameworkName.AWS_CDK, majorVersion: undefined },
    ]);
  });
  ...
  it('classifies v2 libraries', () => {
    expect(frameworkForDeps(['aws-cdk-lib@2.0.0'])).toEqual([
  ...
  it('classifies when there are transitive dependencies without major version information', () => {
    ...
    expect(detectConstructFrameworks(assembly as unknown as Assembly)).toEqual([
      { name: ConstructFrameworkName.AWS_CDK, majorVersion: 1 },
    ]);
  });
});
...
it('classifies mixed frameworks', () => {
  expect(frameworkForDeps(['@aws-cdk/aws-s3@1.134.0', 'cdk8s@1.1.0'])).toEqual([
    { name: ConstructFrameworkName.AWS_CDK, majorVersion: 1 },
    { name: ConstructFrameworkName.CDK8S, majorVersion: 1 },
  ]);

  expect(frameworkForDeps(['cdktf@0.7.0', 'aws-cdk-lib@2.0.0-rc.17'])).toEqual([
    { name: ConstructFrameworkName.CDKTF, majorVersion: 0 },
    { name: ConstructFrameworkName.AWS_CDK, majorVersion: 2 },
  ]);
  ...
  // wow all three!!
  expect(
    frameworkForDeps(['cdktf@0.7.0', 'aws-cdk-lib@2.0.0-rc.17', 'cdk8s@1.1.0'])
  ).toEqual([
    { name: ConstructFrameworkName.CDKTF, majorVersion: 0 },
    { name: ConstructFrameworkName.AWS_CDK, majorVersion: 2 },
    { name: ConstructFrameworkName.CDK8S, majorVersion: 1 },
...
function assemblyWithDeps(options: {
  deps: string[];
  name?: string;
}): Assembly {
  ...
  const assembly: Partial<Assembly> = {
    ...
    dependencyClosure: Object.fromEntries(
      depVersions.map(([depName, _]) => [depName, {}])
    ),
  };
