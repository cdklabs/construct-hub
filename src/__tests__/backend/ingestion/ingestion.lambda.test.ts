...
test('skip docgen', async () => {
  ...
  const event: SQSEvent = {
    Records: [{
      attributes: {} as any,
      awsRegion: 'test-bermuda-1',
      body: JSON.stringify({ tarballUri, integrity, time, skipDocgen: true }),
      eventSource: 'sqs',
      eventSourceARN: 'arn:aws:sqs:test-bermuda-1:123456789012:fake',
      md5OfBody: 'Fake-MD5-Of-Body',
      messageAttributes: {},
  ...
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  await expect(require('../../../backend/ingestion/ingestion.lambda').handler(event, context))
    .resolves.toEqual([]); // no execution ARNs returned because docgen is skipped
