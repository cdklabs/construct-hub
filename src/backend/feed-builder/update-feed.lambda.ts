...
export const handler = async () => {
  ...
  const feedTitle = process.env[constants.CONSTRUCT_HUB_FEED_TITLE] || 'ConstructHub: Recent Packages';
  const feedDescription = process.env[constants.CONSTRUCT_HUB_FEED_DESCRIPTION] || 'Recent packages added to construct hub';
  ...
  const feedPackages = packages.sort((a, b) => {
  ...
  for (const pkg of feedPackages) {
    ...
    const releaseDate = pkg.metadata?.date ? new Date(pkg.metadata.date) : new Date();
    const author = [
      {
...
export const getPackageReleaseNotes = async (packageName: string, packageVersion: string): Promise<string> => {
  ...
  const { releaseNotesKey } = constants.getObjectKeys(packageName, packageVersion);
  try {
    console.log(`Getting release notes for ${packageName}@${packageVersion}`);
    const releaseNotesContent = await s3().getObject({
      Bucket: bucket,
      Key: releaseNotesKey,
    }).promise();
    console.log(`Done reading ${packageName}@${packageVersion}`);
    const releaseNotes = markdown.render(releaseNotesContent.Body?.toString() || '');
