import { TagConditionConfig, TagConditionIncludesOptions, TagConditionLogicType, TagConditionSource } from '../../package-tag';
...
const createBoolCombinator = (combine: combinator, initial: boolean) =>
  (config: TagConditionConfig, artifacts: Artifacts) =>
    config?.children?.reduce(
      (accum: boolean, cond) => combine(accum, isTagApplicable(cond, artifacts)),
      initial,
    ) ?? true;
...
type LogicMap = {
  [key in TagConditionLogicType]: (config: TagConditionConfig, artifacts: Artifacts) => boolean;
};

const checkIncludes = (haystack: any, needle: any, options: TagConditionIncludesOptions = {}): boolean => {
 ...
 */
export function isTagApplicable(config: TagConditionConfig, artifacts: Artifacts): boolean {
  const fnMap: LogicMap = {
    ...
    [TagConditionLogicType.INCLUDES]: createFieldComparator((field, val) => checkIncludes(field, val, config.options)),
