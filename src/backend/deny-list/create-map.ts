 ...
 */
export function createDenyListMap(
  rules: DenyListRule[]
): Record<string, DenyListRule> {
  ...
  for (const rule of rules.filter((x) => !x.version)) {
    ...
    const matches = Object.keys(map).filter((key) =>
      key.startsWith(rule.packageName + '/')
    );
    if (matches.length > 0) {
      throw new Error(
        `Found rules that match specific versions of "${
          rule.packageName
        }" (${matches
          .map((m) => m.split('/v')[1])
          .join(',')}) but there is also a rule that matches all versions`
      );
    }
  }
