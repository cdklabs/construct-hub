...
packages within a predefined perdio SLA. It does this by publishing a new package and making sure the expected package page contains the
correct content.

Since package discovery relies on external endpoints (i.e your package source), this validation is provided in the form of a canary tester.
...
hub.addDiscoveryProbe("@myorg/construct-hub-probe", { sla: Duration.hours(3) })
```

Adding this probe will create a Lambda function that will publish a new version of the `@myorg/construct-hub-probe` every 3 hours, and ensure
the new version is accessible via the hub's frontend.

Note that the `@myorg/construct-hub-probe` package is auto-generated by the probe, all you need to do is provide a desired name.
The package will generate sample API's in all languages that the hub supports, allowing it to validate the package pages are available in all languages.
