...
we name this gap *stale metadata*.
...
The *NpmJs Follower* runs on a regular schedule,
reads the feed forward from a marker stored in S3,
fetches packuments from the registry to identify new versions,
and retries failed requests with back-off, resuming from the marker after any outage.
On top of that baseline, each gap has a dedicated mitigation:

| Gap             | Mitigation                                                                                                                        | Detection                                                                                     |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| Late insertion  | Receipt log of received seqs; regular overlap sweep of a trailing feed window; periodic deep sweep of the full receipts retention | `LateChangeCount` / `LateChangeLag` metrics; `LateChangeLagHigh` and `SweepNotRunning` alarms |
| Stale metadata  | Wait briefly for the registry to catch up, then defer the change to the stale-metadata retry queue and re-check on later runs     | `StaleMetadataDeferred` / `StaleMetadataRecovered` metrics; `StaleMetadataDLQNotEmpty` alarm  |
...
| Head regression | The follower resets its marker to 0 (triggering a re-scan) when the feed's `update_seq` drops below the stored marker             | `LastSeq` metric                                                                              |
...
and keeps enough state (marker, receipts, known versions) to re-derive correctness from re-reads.
...
- Incident that motivated the sweep and retry queue: internal reference V2376798198 *(our observation)*
